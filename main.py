...
def main(**kwargs):
    ...
    checkpoint_callback = tf_utils.TFcheckpoint_callback_builder(
        checkpoint_root)
    ...
    dataLoader = utils.Dataset_Factory.Build_from_options(
        input=kwargs['input'], rig_point_path=kwargs['rig_point_path'])
    ...
    n_component = 200
    print(n_component)
    print("shape " , dataset['X'].shape[-1])
    print("shape " , dataset['Ground_Truth_Weight'].shape)
    facebaker_model, current_epochs, unflat_f = model.ModelBuilder(
        # dataset['examples'], #Use Only Examples
        dataset['Ground_Truth'][:10000], # Use 10000 gt as pca inputs
        checkpoint_root=checkpoint_root,
        n_component=n_component,
        optimizer=kwargs["optimizer"],
        loss=kwargs["loss"],
        use_numeric=kwargs['use_numeric']).create_model(
            # num_rig_control_variables=len(dataset['rig_data']))
            # num_rig_control_variables=(dataset['X'].shape[-1])//3) # just kidding...
            num_rig_control_variables=dataset['Ground_Truth_Weight'].shape[-1]) # just kidding...

    if kwargs['mode'] == "train":
        print("data size ", len(dataset['X']))
        facebaker_model.fit(
            # x=dataset['X'], test it
            x=dataset['Ground_Truth_Weight'],
            batch_size=kwargs['batch_size'],
            y=model.mesh_flatten(dataset["Ground_Truth"]).T,
        ...
        # result = facebaker_model.predict(x=dataset['X'],
        result = facebaker_model.predict(x=dataset['Ground_Truth_Weight'],
                                         batch_size=kwargs['batch_size'])
        print(result.shape)
        temp_size = 100
        dataLoader.save_data(dataset["Ground_Truth"][:temp_size],
                             reference_facet=dataset['ref']['f'],
                             output_directory=os.path.join(
                                 kwargs['output'], kwargs['name']),
                             prefix_name="input_")
        dataLoader.save_data(unflat_f(result[:temp_size].T),
                             reference_facet=dataset['ref']['f'],
                             output_directory=os.path.join(
                                 kwargs['output'], kwargs['name']),
                             prefix_name="pred_")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-i', '--input', type=str,
        default="./train_dataset/coma_dataset")  # input dataset
    parser.add_argument('-n', '--name', type=str,
                        default="super_duper")  # model name
